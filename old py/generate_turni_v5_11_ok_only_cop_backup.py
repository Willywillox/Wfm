...
def compute_tight_capacity_targets(req_pre: pd.DataFrame, ris: pd.DataFrame, durations_by_emp: dict,
    ...
    overcap_penalty = {}
    for g in giorni:
        pct = DAY_OVERCAP_PERCENT.get(g, DEFAULT_OVERCAP_PERCENT)
        limit = max(0.0, demand_by_day.get(g, 0.0) * pct) if g not in zero_demand_days else 0.0
        overcap_limit[g] = limit
        overcap_penalty[g] = DAY_OVERCAP_PENALTY.get(g, DEFAULT_OVERCAP_PENALTY)
        ...
        for emp in remaining_people():
            ...
            if best_sid is not None:
                ...
                if fallback_sid is not None:
                    apply_assignment(emp, fallback_day, fallback_sid)
                    assigned_flag = True
                else:
    ...
    def ensure_required_days():
        ...
        while True:
            ...
            for emp in missing:
                ...
                if sid_sel is None:
                    continue
                apply_assignment(emp, day_sel, sid_sel)
                progress = True
            if not progress:
