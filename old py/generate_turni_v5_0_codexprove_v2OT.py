...
def infer_personal_params_from_risorse(ris: pd.DataFrame):
    ...
    for _, row in ris.iterrows():
        ...
        if ot_disp_col is not None and pd.notna(raw_ot) and str(raw_ot).strip() != '':
            try:
                ot_minutes = float(str(raw_ot).replace(',', '.')) * 60.0
            except Exception:
                ot_minutes = 0.0
        ...
        for day, col in ot_day_cols.items():
            ...
            if col is not None and pd.notna(val) and str(val).strip() != '':
                try:
                    minutes = float(str(val).replace(',', '.')) * 60.0
                except Exception:
                    minutes = 0.0
...
def assegnazione_tight_capacity(req: pd.DataFrame, turni_cand: pd.DataFrame, ris: pd.DataFrame,\n                                shift_by_emp: dict, rest_target_by_emp: dict, durations_by_emp: dict,\n                                forced_off: dict, forced_on: dict, ot_minutes_by_emp: dict, ot_split_by_emp: dict, ot_daily_minutes_by_emp: dict, prefer_phases=(15, 45), strict_phase=False):
    ...
    def allocate_overtime():
        ...
        for (emp, day), meta in assignment_details.items():
            minutes = float(ot_daily_minutes_by_emp.get(emp, {}).get(day, 0.0) or 0.0)
            if minutes <= 0:
                continue
            if slot_size <= 0:
                continue
            slots_exact = minutes / slot_size
            slots_needed = int(round(slots_exact))
            if abs(slots_needed * slot_size - minutes) > 1e-6:
                slots_needed = int(math.ceil(minutes / slot_size - 1e-9))
            if slots_needed <= 0 and minutes > 0:
                slots_needed = 1
            if slots_needed <= 0:
                continue
        ...
        def can_extend_start(emp: str, meta: dict, steps: int) -> bool:
            if steps <= 0:
                return False
            if not split_flags[emp] and meta.get('ot_end_slots', 0) > 0:
                return False
        ...
        def can_extend_end(emp: str, meta: dict, steps: int) -> bool:
            if steps <= 0:
                return False
            if not split_flags[emp] and meta.get('ot_start_slots', 0) > 0:
                return False
        ...
        def perform_extend(emp: str, day: str, direction: str, steps: int) -> bool:
            ...
            if direction == 'start':
                if not can_extend_start(emp, meta, steps):
                    return False
                for _ in range(steps):
                    ...
                    meta['ot_direction'] = 'start' if meta['ot_direction'] in (None, 'start') else 'both'
                    meta['extra_slots'].add(new_slot)
                    if new_slot in current_coverage[day]:
                        current_coverage[day][new_slot] += 1
                return True
            if direction == 'end':
                if not can_extend_end(emp, meta, steps):
                    return False
                for _ in range(steps):
                    ...
                    meta['total_ot_minutes'] = meta['ot_minutes_start'] + meta['ot_minutes_end']
                    meta['ot_direction'] = 'end' if meta['ot_direction'] in (None, 'end') else 'both'
                    meta['extra_slots'].add(new_slot)
                    if new_slot in current_coverage[day]:
                        current_coverage[day][new_slot] += 1
                return True
    ...
    for emp, day, sid in assignments:
        ...
        start_minute = meta.get('actual_start_min', shift_start_min[sid]) if meta else shift_start_min[sid]
        minute_distribution[start_minute % 60] += 1
...
def crea_output(assignments, turni_cand: pd.DataFrame, ris: pd.DataFrame,
    ...
    name_map = dict(zip(ris['id dipendente'], ris['Nome']))
    start_min_map = dict(zip(turni_cand['id turno'], turni_cand['start_min']))
    end_min_map = dict(zip(turni_cand['id turno'], turni_cand['end_min']))
    day_name_map = {'Lun':'LunedǪ','Mar':'MartedǪ','Mer':'MercoledǪ','Gio':'GiovedǪ','Ven':'VenerdǪ','Sab':'Sabato','Dom':'Domenica'}

    rows = []
    for emp, day, turno in assignments:
        meta = details_map.get((emp, day), {})
        base_start = int(start_min_map.get(turno, 0))
        base_end = int(end_min_map.get(turno, base_start))
        actual_start = int(meta.get('actual_start_min', base_start))
        actual_end = int(meta.get('actual_end_min', base_end))
        duration = actual_end - actual_start
        if duration <= 0:
            duration = (actual_end + 24 * 60) - actual_start
        ot_minutes_start = int(meta.get('ot_minutes_start', 0) or 0)
        ot_minutes_end = int(meta.get('ot_minutes_end', 0) or 0)
        ot_total = int(meta.get('total_ot_minutes', 0) or 0)
        note_parts = []
        if ot_minutes_start:
            note_parts.append(f"Anticipa {ot_minutes_start}m")
        if ot_minutes_end:
            note_parts.append(f"Prolunga {ot_minutes_end}m")
        ot_ingresso = ''
        if ot_minutes_start:
            ot_ingresso = f"{_from_minutes(actual_start)}-{_from_minutes(base_start)}"
        ot_uscita = ''
        if ot_minutes_end:
            ot_uscita = f"{_from_minutes(base_end)}-{_from_minutes(actual_end)}"
        rows.append({
            'emp_id': emp,
            'Nome': name_map.get(emp, emp),
            'Key': f"{name_map.get(emp, emp)} [{emp}]",
            'Giorno': day_name_map[day],
            'Inizio': _from_minutes(actual_start),
            'Fine': _from_minutes(actual_end),
            'Durata_min': duration,
            'TurnoID': turno,
            'OT_min': ot_total,
            'OT_note': ' + '.join(note_parts),
            'OT_ingresso': ot_ingresso,
            'OT_uscita': ot_uscita
        })
    df_ass = pd.DataFrame(rows)

    all_days_long = ['LunedǪ','MartedǪ','MercoledǪ','GiovedǪ','VenerdǪ','Sabato','Domenica']
    ...
    rows_cov = []
    long_map = {'Lun':'LunedǪ','Mar':'MartedǪ','Mer':'MercoledǪ','Gio':'GiovedǪ','Ven':'VenerdǪ','Sab':'Sabato','Dom':'Domenica'}
    for day in giorni_short:
        for s in slot_list:
            rows_cov.append({
                'Giorno': long_map[day],
