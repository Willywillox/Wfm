...
def assegnazione_tight_capacity(req: pd.DataFrame, turni_cand: pd.DataFrame, ris: pd.DataFrame,
    ...
    """Assegna i turni rispettando domanda e riposi contrattuali."""
    giorni = ['Lun','Mar','Mer','Gio','Ven','Sab','Dom']
    ...
    phase_targets = {0: 0.20, 15: 0.30, 30: 0.20, 45: 0.30}
    ...
    def minute_preference_score(start_min: int) -> float:
        ...
        expected = phase_targets[minute] * total_assigned if total_assigned > 0 else 0.0
        actual = minute_distribution.get(minute, 0)
        diff = expected - actual
        scale = 90.0 if strict_phase else 45.0
        bonus = 25.0 if minute in prefer_phases else -12.0
    ...
    def day_balance_bonus(day: str) -> float:
        ...
        return (desired - actual) * (70.0 if strict_phase else 35.0)

    def shift_value(day: str, sid: str, allow_overcapacity: bool = False) -> float:
        ...
        for slot in shift_slots.get(sid, []):
            ...
            if gap > 0:
                ...
                penalty = abs(gap)
                damp = 12.0 if allow_overcapacity else 35.0
                if demand <= 0:
                    damp *= 3.0
                score -= penalty * damp
    ...
    while remaining_people() and safety < 12000:
        ...
        progressed = False
        for gap_value, day, target_slot in critical_gaps[:20]:
            candidates = []
            ...
            if not candidates:
                continue
            val, _, _, emp, sid = max(candidates)
        ...
        for emp in remaining_people():
            ...
            for day in giorni_validi:
                if (emp, day) in assigned_once or day in forced_off.get(emp, set()):
                    continue
                for sid in shift_by_emp.get(emp, []):
                    val = shift_value(day, sid, allow_overcapacity=True)
                    if val <= -1e4:
                        continue
                    key = (val, -shift_start_min[sid])
                    if best is None or key > best:
    ...
    for emp in remaining_people():
        infeasible.append((emp, 'Impossibile assegnare il numero di turni richiesto dalle risorse (post-loop).'))
    ...
    for minute in sorted(minute_distribution.keys()):
        ...
        marker = '*' if minute in prefer_phases else ''
        print(f"  :{minute:02d} -> {count} turni ({pct:.1f}%) {marker}")
    preferred = sum(minute_distribution.get(m, 0) for m in prefer_phases)
    if total_assignments:
        print(f"\nTurni ai minuti preferiti {prefer_phases}: {preferred}/{total_assignments} ({preferred/total_assignments*100:.1f}%)")
...
def build_warning_summary(riposi_info, infeasible, ris):
...
def main():
    ...
    df_warn = build_warning_summary(riposi_info, infeasible, ris)
