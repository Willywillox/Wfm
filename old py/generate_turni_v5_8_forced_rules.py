...
def assegnazione_tight_capacity(
    ...
    def allocate_overtime():
        ...
        def ensure_detached_meta(emp: str, day: str):
            meta = assignment_details.get((emp, day))
            if meta is not None:
                return meta
            demand_slots = [slot for slot in slot_list if slot_has_demand(day, slot)]
            if not demand_slots:
                return None
            start_slot = max(demand_slots, key=lambda slot: demand_by_slot[day].get(slot, 0.0))
            meta = {
                'sid': None,
        ...
        def allocate_detached_block(emp: str, meta: dict, day: str, steps: int) -> bool:
            if steps <= 0:
                return True
            meta = ensure_detached_meta(emp, day)
            if meta is None:
                return False
            meta.setdefault('extra_slots', set())
            ...
            for slot in slot_list:
                ...
                score = 0.0
                for _ in range(steps):
                    if cur not in slot_set or cur in forbidden or not slot_has_demand(day, cur):
                        ok = False
                        break
            ...
            for _ in range(steps):
                ...
                if cur in current_coverage[day]:
        ...
        def force_extend_forced(emp: str, day: str, slots_needed: int) -> bool:
            split = split_flags.get(emp, False)
            base_start = int(meta.get('base_start', meta.get('actual_start_min', slot_list[0])))
            base_end = int(meta.get('base_end', meta.get('actual_end_min', slot_list[0])))
            has_base = base_end > base_start
            
            def score_attached(side, steps):
                score = 0.0
                if side == 'end':
                    cur = base_end
                    for _ in range(steps):
                    ...
                    cur = base_start - steps * slot_size
                    for _ in range(steps):
                        score += demand_by_slot[day].get(cur, 0.0) - current_coverage[day][cur]
                        cur += slot_size
                return score
            ...
            if not has_base:
                return allocate_detached_block(emp, meta, day, slots_needed)
            
            if not split:
                sc_end = score_attached('end', slots_needed)
                sc_start = score_attached('start', slots_needed)
                direction = 'end' if sc_end >= sc_start else 'start'
                cur = base_end if direction == 'end' else base_start - slots_needed * slot_size
                for _ in range(slots_needed):
                    ...
                    cur += slot_size
                if direction == 'end':
                    meta['actual_end_min'] = base_end + slots_needed * slot_size
                    meta['ot_minutes_end'] = slots_needed * slot_size
                    meta['ot_end_slots'] = slots_needed
                else:
                    meta['actual_start_min'] = base_start - slots_needed * slot_size
                    meta['ot_minutes_start'] = slots_needed * slot_size
                    meta['ot_start_slots'] = slots_needed
                meta['total_ot_minutes'] = slots_needed * slot_size
                return True
            else:
                return allocate_detached_block(emp, meta, day, slots_needed)
        ...
        for day in ['Lun', 'Mar', 'Mer', 'Gio', 'Ven', 'Sab', 'Dom']:
                ...
                while sum(day_required.values()) > 0:
                    ...
                    for slot in slot_list:
                        if not slot_has_demand(day, slot):
                            continue
                        demand_val = demand_by_slot[day].get(slot, 0.0)
                        gap = demand_val - current_coverage[day][slot]
                        if gap > 1e-6:
                            metric = gap + demand_val * 1e-3
                        elif demand_val > 1e-6:
                            metric = demand_val * 1e-3
                        else:
                            continue
                        if metric <= 1e-6:
                            continue
                        gap_slots.append((metric, slot))
                    gap_slots.sort(reverse=True)

                    for metric, slot in gap_slots:
                        best_plan = None
                        for emp, meta in day_assigns:
                            remaining = day_required.get(emp, 0)
                            if remaining <= 0:
                            ...
                            candidate = (metric, -steps, emp, direction, steps)
            ...
            for emp in targets.keys():
                ...
                if residual > 0 and allow_ot_overcap:
                    slots_missing = residual // slot_size
                    if slots_missing > 0:
                        success = False
                        if meta is not None:
                            success = force_extend_forced(emp, day, slots_missing)
                        elif split_flags.get(emp, False):
                            success = allocate_detached_block(emp, ensure_detached_meta(emp, day), slots_missing)
                        if success:
                            meta = assignment_details.get((emp, day))
                            actual_minutes = int(meta.get('total_ot_minutes', 0) or 0) if meta else 0
                            residual = expected_minutes - actual_minutes
