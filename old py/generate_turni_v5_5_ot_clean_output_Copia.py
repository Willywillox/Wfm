...
def assegnazione_tight_capacity(
    ...
    def allocate_overtime():
        ...
        def force_extend_forced(emp: str, day: str, slots_needed: int) -> bool:
            ...
            meta = assignment_details.get((emp, day))
            if allow_ot_overcap:
                print(f'DEBUG FORCE -> {emp} {day} slots {slots_needed} meta={'Y' if meta else 'N'}')
            if meta is None:
                return False
            ...
            def direction_capacity(direction: str):
                ...
                while True:
                    ...
                    if steps >= slots_needed and not split:
                        # For non split workers we only need to know up to slots_needed
                        break
            ...
            if not split:
                ...
                if cap_end >= slots_needed:
                    choices.append(('end', sum(dem_end[:slots_needed])))
                if cap_start >= slots_needed:
                    choices.append(('start', sum(dem_start[:slots_needed])))
                if not choices:
                    return False
